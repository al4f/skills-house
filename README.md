...
### Install via npx (from `main`)

Built skills live in `skills-dist/`. Install from the default branch with the [skills](https://github.com/vercel-labs/skills) CLI:

```bash
# One skill
npx skills add al4f/skills-house/skills-dist --skill brainstorming -g -y

# All skills in skills-dist/
npx skills add al4f/skills-house/skills-dist -g -y
```

`-g` installs globally (user-level). Omit it to install into the current project only.
